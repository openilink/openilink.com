...
下面是 `openilink` 组织当前公开的仓库列表。这个页面用于把官网、Hub、SDK 与生态项目放到同一个索引面里，避免入口分散。当前同步到的公开仓库总数为 10 个，其中 SDK 仓库为 6 个。
...
| [openilink/openclaw-channel-openilink](https://github.com/openilink/openclaw-channel-openilink) | 渠道集成 | - | OpenClaw 渠道相关集成仓库 |
...
- 官网层：`openilink.com`
- 中枢层：`openilink-hub`
- 渠道 / 生态层：`openilink-tg`、`openclaw-channel-openilink`
- SDK 层：6 个不同运行时的接入包
